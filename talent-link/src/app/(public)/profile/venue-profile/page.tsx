...
import { MapPin, Phone, Mail, Globe, Calendar, Loader } from 'lucide-react'
...
const VenueProfile = () => {
  ...
  useEffect(() => {
    const loadVenue = async () => {
      ...
      try {
        ...
        if (me.role !== 'venue') {
          const errorMsg = 'Bạn không có quyền xem trang này'
          setError(errorMsg)
  ...
  if (error || !venue) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <p className="text-center text-muted-foreground">{error || 'Không tìm thấy hồ sơ'}</p>
      </div>
    )
  }
